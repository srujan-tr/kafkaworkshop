...
echo "Starting Kafka in background with KAFKA_HEAP_OPTS='${KAFKA_HEAP_OPTS}'"
"${KAFKA_HOME}/bin/kafka-server-start.sh" -daemon "${CFG}"
echo "Kafka started. Use status-kafka.sh to check status."
